v2.0.0 发布日期：2026-09-20

- **破坏性变更**：移除图床（图片上传）能力，项目收窄为纯浏览器工具
- 移除 `Lib` 的 `useImageHosting` prop 及对应的 TypeScript 类型声明
- 移除「格式-图片」菜单项、右键菜单图片项、`Ctrl/Cmd+Alt+I` 快捷键、编辑器拖拽与粘贴上传
- 移除「设置-上传图片时包含名称」选项
- 移除 SM.MS / 阿里云 OSS / 自定义图床的实现与配置界面
- 移除依赖：`ali-oss`、`axios`、`@storybook/react` 及 `stories/` 目录
- 图片改为填写可公开访问的 https 链接
